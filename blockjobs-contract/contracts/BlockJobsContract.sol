...
contract BlockJobsContract {
    ...
    constructor(
        address payable _tokenContractAddress,
    ...
    struct Career {
        ...
        uint256 stDt; // 근무 시작일
        uint256 fnsDt; // 근무 종료일
        uint256 status; // 승인 처리 상태  (0-대기, 1-승인, 2-거절)
    }
    ...
    struct Review {
        ...
        uint256 createDt; // 작성 일자
        string nftUri; // nft 등록시 주소
    }

    uint256 public CareerTotalSupply = 1;
    uint256 public ReviewTotalSupply = 1;

    mapping(uint256 => Career) public Career_mapping;
    mapping(address => uint256[]) public CareerByWorker_mapping;
    mapping(address => uint256[]) public CareerByCompany_mapping;

    mapping(uint256 => Review) public Review_mapping;
    mapping(address => uint256[]) public ReviewByWriter_mapping;
    mapping(address => uint256[]) public ReviewByCompany_mapping;

    event createCareer_event(address writer, uint256 careerId);
    event approveCareer_event(uint256 careerId);
    event transferFrom_event(address from, address to, uint256 amout);

    // 회원관리
    // Coin Approve;
    function approveUser(uint256 _amount) public payable {
        BlockJobCoinAddress.CoinApprove(
            msg.sender,
            address(this),
            _amount * (10**uint256(BlockJobCoinAddress.decimals()))
        );
    }
    ...
    function transferFrom(address _to, uint256 _amount) public payable {
        BlockJobCoinAddress.transferFrom(
            msg.sender,
            _to,
            _amount * (10**uint256(BlockJobCoinAddress.decimals()))
        );
    ...
    function Buy() public payable {
        ...
        uint256 bjcBalance = BlockJobCoinAddress.balanceOf(
            BlockJobCoinAddress._adminAddress()
        );
    ...
    function createCareer(
        ...
        // Create Struct Data;
        Career_mapping[CareerTotalSupply] = Career(
            CareerTotalSupply,
            _role,
            _description,
            msg.sender,
    ...
    function approveCareer(
        ...
        require(
            Career_mapping[_careerId].company == msg.sender,
            "Not Matching Company Address"
        );
        ...
        if (_status == 2) {
            BlockJobCoinAddress.transferFrom(
                _deployAddress,
                Career_mapping[_careerId].worker,
                ((_amount * uint256(10**BlockJobCoinAddress.decimals())) * 3) /
                    10
            );
        }
        // 승인 시에 근로자가 등록할 때 admin에게 지불한 값 80%는 기업에게 배분 (가스비가...?)
            ...
            BlockJobCoinAddress.transferFrom(
                _deployAddress,
                msg.sender,
                ((_amount * uint256(10**BlockJobCoinAddress.decimals())) * 8) /
                    10
            );
        }
    ...
    function createReview(
        ...
        uint256 userReviewCount = ReviewByWriter_mapping[msg.sender].length;

        // 이미 리뷰를 작성한 유저는 재작성 불가
        for (uint256 i = 0; i < userReviewCount; i++) {
            uint256 idx = ReviewByWriter_mapping[msg.sender][i];
            if (Review_mapping[idx].company == _company) {
                revert("Already Create Review");
            }
        }

        Review_mapping[ReviewTotalSupply] = Review(
            ReviewTotalSupply,
            _title,
            _content,
            _company,
        ...
        BlockJobCoinAddress.transferFrom(
            _deployAddress,
            msg.sender,
            _amount * uint256(10**BlockJobCoinAddress.decimals())
        );
    ...
    // 리뷰 조회 (회사 기준)
    function getReviewByWriter(address _writer)
        public
        view
        returns (Review[] memory)
    {
        // @Exception
        // @Logic
        Review[] memory result = new Review[](
            ReviewByWriter_mapping[_writer].length
        );

        // Review Mapping 순회
        for (uint256 i = 1; i < ReviewTotalSupply; i++) {
            ...
            if (Review_mapping[i].writer == _writer) {
                ...
                for (
                    uint256 x = 0;
                    x < ReviewByWriter_mapping[_writer].length;
                    x++
                ) {
                    if (result[x].id != ReviewByWriter_mapping[_writer][x]) {
                        result[x] = Review(
                            Review_mapping[i].id,
                            Review_mapping[i].title,
                            Review_mapping[i].content,
                            Review_mapping[i].company,
                            Review_mapping[i].writer,
                            Review_mapping[i].createDt,
                            Review_mapping[i].nftUri
                        );
                    }
                }
            }
    ...
    function getReviewByCompany(address _company)
        public
        view
        returns (Review[] memory)
    {
        ...
        // @Logic
        Review[] memory result = new Review[](
            ReviewByCompany_mapping[_company].length
        );
        ...
        for (uint256 i = 1; i < ReviewTotalSupply; i++) {
            ...
            if (Review_mapping[i].company == _company) {
                ...
                for (
                    uint256 x = 0;
                    x < ReviewByCompany_mapping[_company].length;
                    x++
                ) {
    ...
    // 커리어 조회
    function getCareerDetail(uint256 _careerId)
        public
        view
        returns (
            string memory,
        ...
        )
    {
        ...
        return (
            Career_mapping[_careerId].description,
            Career_mapping[_careerId].roles,
            Career_mapping[_careerId].worker,
            Career_mapping[_careerId].company,
            Career_mapping[_careerId].stDt,
            Career_mapping[_careerId].fnsDt,
            Career_mapping[_careerId].status
        );
    }

    // 근로자 기준으로 커리어목록 가져오기
    function getCareerByWorker(address _worker)
        public
        view
        returns (Career[] memory)
    {
        // @Exception
        // @Logic
        ...
        for (uint256 i = 1; i < CareerTotalSupply; i++) {
            // 커리어매핑 Struct 작성자 확인;
            if (Career_mapping[i].worker == _worker) {
                for (
                    uint256 x = 0;
                    x < CareerByWorker_mapping[_worker].length;
                    x++
                ) {
                    if (result[x].id != CareerByWorker_mapping[_worker][x]) {
                        result[x] = Career(
                            Career_mapping[i].id,
                            Career_mapping[i].roles,
                            Career_mapping[i].description,
                            Career_mapping[i].worker,
                            Career_mapping[i].company,
                            Career_mapping[i].stDt,
                            Career_mapping[i].fnsDt,
                            Career_mapping[i].status
                        );
                    }
                }
            }
    ...
    function getCareerByCompany(address _company)
        public
        view
        returns (Career[] memory)
    {
        // @Exception
        // @Logic
        Career[] memory result = new Career[](
            CareerByCompany_mapping[_company].length
        );
        ...
        for (uint256 i = 1; i < CareerTotalSupply; i++) {
            ...
            if (Career_mapping[i].company == _company) {
                for (
                    uint256 x = 0;
                    x < CareerByCompany_mapping[_company].length;
                    x++
                ) {
    ...
    function mintNft(
        ...
        require(
            Review_mapping[_reviewId].writer == _owner,
            "Bad Reuqest owner address"
        );
    ...
    // NFT 보유자 주소;
    function getNftOwnerOf(uint256 _nftId) public view returns (address) {
        return BlockJobsNftAddress.ownerOf(_nftId);
    }

    // NFT URI 반환
    function getNftTokenUri(uint256 _nftId)
        public
        view
